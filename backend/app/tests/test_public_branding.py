...
class TestPublicBrandingAPI(unittest.TestCase):
    ...
    def setUp(self):
        # Clear domain mappings and onboardings for our test user
        cls_onboarding = self.db.query(Onboarding).filter(Onboarding.user_id == self.test_user_id).first()
        if cls_onboarding:
            self.db.query(DomainMapping).filter(DomainMapping.onboarding_id == cls_onboarding.id).delete()
        self.db.query(Onboarding).filter(Onboarding.user_id == self.test_user_id).delete()
        self.db.commit()

    def test_get_branding_success(self):
        # 1. Create onboarding with branding config
    ...
    def test_get_branding_missing_config_404(self):
        ...
        # 2. Create domain mapping pointing to this onboarding_id
        mapping = DomainMapping(
        ...
        try:
            ...
            self.db.query(Onboarding).filter(Onboarding.id == onboarding.id).delete()
            self.db.commit()
